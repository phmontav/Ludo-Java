...
public class GridTest {
	...
	public static void main(String[] args) {
		...
		for(int linha = 0; linha <15; linha++) {
			for(int col = 0; col<15; col++) {
				if((col==5 || col==7 || col==9)&&(linha<=5 || linha>=9)) {
					tabuleiro.add(caminho[index[linha][col]] = new Casa(index[linha][col]));
				}
				else {
		...
		camadas.add(new Origem(1));
		camadas.add(new Origem(2));
		camadas.add(new Origem(3));
		camadas.add(new Origem(4));
